...
| **cursor** | Runtime anfitrión desde 032, pines por rol desde 034. Recibe los 28 roles como subagentes nativos (`~/.cursor/agents/*.md`, fuente `Global/cursor/agents/`), las skills canónicas (`~/.cursor/skills/<nombre>/SKILL.md`) y, **por proyecto**, la doctrina como regla siempre activa (`.cursor/rules/00-harness.mdc`, `alwaysApply: true`) más los slash commands (`.cursor/commands/`) — reglas y comandos no tienen equivalente global en Cursor. Cada rol declara `model:` desde `models.toml` (`cursor=`): los `code-rw` (incl. `repair-agent`) pinnean el barato medido; los jueces pinnean otra familia. `inherit` en un reviewer muere en generate (Cursor inherit es el modelo del padre, no una familia distinta). Cursor sigue **fuera** de `RUNTIMES`; `--route-decide` sigue prohibido en este anfitrión. **Cero hooks de evento**: `hooks.json` no se instala, así que `coord_policy.py` NO gobierna en Cursor — quien gobierna es el modelo de permisos del propio Cursor. *(Pines medidos 2026-08-19 contra https://cursor.com/docs/subagents y https://cursor.com/docs/models-and-pricing; ver ADR-0063 y `docs/specs/034-cuota-organica-y-writer-barato/spec.md`.)* |
