# Global Harness Rules (Cursor)

These rules apply to every session unless a project `AGENTS.md` is more specific. In Cursor they are
delivered as a project rule (`.cursor/rules/00-harness.mdc`, `alwaysApply: true`), because Cursor has no
user-level rules file — verified 2026-08-18 against <https://cursor.com/docs/context/rules>.

## What Cursor gives this harness, and what it does not
- **Subagents are native.** The roles live in `~/.cursor/agents/*.md` and are delegated to by name
