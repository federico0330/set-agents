...
The precedence is unequivocal: first apply an answer already resolved by ADR-0037; only then apply
the selected posture to an action that ADR-0037 did not resolve. Thus no posture reopens a decided
repair, gate, or next approved package, and an unresolved proposed action that both mutates and
delegates still has three distinct outcomes. Read and obey this runtime-consumed contract:

POSTURA_RUNTIME_CONTRACT_V1
```text
precedence: adr_0037_resolved > postura
adr_0037_resolved: execute_without_asking
unresolved_mutating_or_delegating_action:
...
`act_on_your_own` means use MCPs, CLIs, and skills autonomously and narrate by milestone.
`propose_and_wait_for_explicit_confirmation_before_mutation` means name the exact mutating action
in one line and wait for explicit confirmation in that turn — never execute it in the same message
that proposes it. `ask_and_wait_before_every_delegation` means ask and wait before EVERY subagent
spawn, mutating or not. This is the decision channel the orchestrator consumes from `config.toml`,
not a separate test-only policy helper. Changing posture is editing `config.toml`
(`set_agents_app.py --postura <valor>`, or the human operator) — never a reinstall; `--posturas`
prints this same table as a screen.
