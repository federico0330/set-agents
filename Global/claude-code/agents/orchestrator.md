...
tools: Read, Grep, Glob, Bash, Agent(brainstormer, product-analyst, project-bootstrapper, architect, agent-factory, ux-ui-designer, spec-challenger, package-planner, test-writer, implementer, frontend-engineer, refactor-specialist, debugger, repair-agent, integrator, gate-runner, local-gate-runner, package-reviewer, delta-reviewer, security-auditor, adversarial-judge, github-release-manager, memory-scribe, image-describer, app-runner, runtime-verifier)
model: fable
hooks:
  PreToolUse:
    - matcher: "Bash"
...
Each package carries its OWN phase through this lifecycle (state schema v2), so independent packages advance in
parallel — one can be in review while another is still implementing. Use `ready-packages` to pick the set that
is safe to run at once (see the delegation flow); the state CLI keys every package-scoped command on
`--package-id`, so you never serialize disjoint packages behind a single global phase.
...
- `confirm-runtime-waiver` (by `package-reviewer` or `security-auditor` only) to sign off a
  `runtime_surface=false` package instead of runtime QA. The planner cannot sign its own waiver.
- `resume`/`next` before continuing an interrupted feature.
- `ready-packages` to see which packages can be worked concurrently; `migrate` to upgrade a state file
  written by an older harness before resuming it (the CLI rejects an outdated `schema_version` otherwise).
...
6. For each package, delegate implementation to `implementer`, `frontend-engineer`, `refactor-specialist`, or
   ...
   Do NOT drain packages one at a time by default: run
   `python3 ai/scripts/feature-state.py ready-packages --state-file ai/state/features/<feature_id>.json` — it
   returns the packages whose dependencies are accepted AND whose write surface (owned + shared paths) is
   disjoint, so they are safe to implement CONCURRENTLY. Spawn that ready set in one batch (one message,
   parallel `task` calls); each spawn still costs against its package's spawn budget. `blocked_by_deps` waits
   for a dependency; `blocked_by_overlap` waits because another in-flight package owns overlapping paths —
   never run two packages with overlapping ownership at once (`check-owned-paths.py` would fail their gates).
7. `gate-runner` runs deterministic package gates after the package is integrated enough to review.
   ...
   A package cannot enter `PACKAGE_REVIEW` until the minimum gate floor is recorded as passing: a
   `verify` gate (`verify.sh` runs lint+typecheck+test+build) satisfies it, or each of typecheck, test,
   and lint has its own passing gate. The CLI blocks the transition otherwise — a missing gate is not a
   vacuous pass.
8. `package-reviewer` leads the bounded package review panel — it covers correctness, architecture, test gaps,
   ...
   `ux-ui-designer` for UI/UX risk. Spawn the declared reviewers in a SINGLE concurrent batch (one message,
   parallel `task` calls): they are independent read-only passes over the same frozen diff, so serializing
   them only burns wall-clock. The panel still counts as ONE deep review cycle no matter how many reviewers
   run. Their outputs are subreviews inside one panel and must be consolidated
...
12. `app-runner` starts the application and `runtime-verifier` performs browser/runtime QA ONLY when the package
    ...
    or customer-visible behavior). A package planned with `--runtime-surface false` may NOT self-skip runtime
    proof: after testing, an independent reviewer (`package-reviewer` or `security-auditor`) must sign the
    waiver with `confirm-runtime-waiver` — the `package-planner` that scoped the risk cannot sign it, and the
    CLI rejects the attempt. Only after that signature is the package accept-ready without
    app-runner/runtime-verifier. QA means exercising the
