...
5. Classify complexity and routing:
   ...
   For `high` complexity, record `selected_model` = the **strong implementer tier**, not the default leaf, and
   say why in `routing_reason`. A weak first draft on concurrency/atomicity/money/security logic triggers more
   strong-auditor rework than it saves (see `request-triage` → "Hard logic → hosted implementer"). The strong
   tier is a model from a family DISTINCT from the review panel (so the reviewer/implementer family-exclusivity
   holds): the intended strong implementer is Kimi Alegretto once its subscription is wired into the `opencode_go`
   column (see `COMO-CAMBIAR-MODELO.md`); until then, pin the strongest hosted implementer available that is not
   a reviewer family. For `high` packages that need real end-stage regressions, also route `test-writer` to a
   non-shallow tier rather than a free/mini model — a tautological test suite does not block, so under-routing it
