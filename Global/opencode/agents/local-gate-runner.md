...
- Allowed: `python3 -m py_compile` for `ai/scripts/feature-state.py` and `ai/scripts/check-owned-paths.py`; `python3 --help` for either script; `check-owned-paths.py` with `--state-file`, `--package-id`, and `--baseline`; `git diff --check`; and `feature-state.py record-gate` only with `--state-file ai/state/002-local-uat-identities-and-feature-state.json`.
- Never edit files or invoke any other command. The sole write exception is that `record-gate` invocation to `ai/state/002-local-uat-identities-and-feature-state.json`. In particular, do not access `.env` files, use network tools, Docker, installation commands, mutating Git commands, shell composition, or MCPs.
