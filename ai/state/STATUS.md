...
Actualizado: 2026-07-27T14:49:56+00:00
...
| 005-portable-harness | feature | PACKAGE_PLANNING | - | 0/0 | 0/12 | 0/2 | 0 | - | PACKAGE_IMPLEMENTATION | 2026-07-27T14:49:56+00:00 init |
...
Ingeniería: PACKAGE_IMPLEMENTATION P3-pi-lane spawn 1/12 (Claude in-session): T-301 ADR-0007 + managed pinned pi install + doctor --harness pi; T-302 minimal pi target generate/install; T-303 set_agents_spawn.py CLI-subprocess spawner (pi --model provider/model --print --mode json --no-session, lifecycle incl crash=>failure, decided-model verification); T-304 spawn guards as flags (fresh ctx, --no-extensions depth-0, read-only until green); T-305 (pi,openai-codex)+(pi,anthropic) probe pairs + parsers (pi --list-models + auth.json key-set, positives-only) + PI_SIMULATION_ONLY flip (service.py:132 gated on doctor green) + model-id map (openai-codex identity; anthropic opus->claude-opus-4-8/sonnet->claude-sonnet-5/haiku->claude-haiku-4-5). Baseline ced2caa. Pi authenticated (anthropic+openai-codex).
